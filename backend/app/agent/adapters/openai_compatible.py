"""The chat-completions shape, shared by every provider that speaks it.

Groq and Gemini both expose an OpenAI-compatible surface, so the wire handling --
bounding the response, parsing tool calls as JSON, imposing the action class -- is
written once here and specialised by a subclass that declares its endpoint. One code
path means a defect fixed for one provider is fixed for both, and it means the Groq
suite that already exercises this logic keeps exercising it after the extraction.

The destination is pinned by the subclass, never configured. An endpoint that an
environment variable could move is an endpoint that can be pointed at an attacker's
server -- and it would carry the API key with it on the first request. Each subclass
is validated at import by ``__init_subclass__``, so a new provider cannot be added
...
# reaching a model provider should be as quick as reaching anything else.
CALL_TIMEOUT_SECONDS: Final = 120.0

# A completion bounded at a few tens of thousands of tokens cannot honestly exceed a
# couple of megabytes once JSON-encoded. The shared 12 MiB MCP ceiling would let a
# misbehaving provider push forty times more than that into memory before anything
# noticed.
...
def _tool_calls_of(
    ...
    for raw_call in raw_calls:
        ...
        if allowed_tool_names and name not in allowed_tool_names:
            ...
            logger.info(
                "The model proposed a tool that was not offered; the loop will refuse it",
                extra={"llm_provider": provider_label, "tool_name": name},
            )
...
class OpenAICompatibleLLMProvider:
    """Chat completions against a pinned endpoint. The API key never leaves here.

    Abstract only by convention: a subclass supplies ``ENDPOINT``, ``PROVIDER_LABEL``
    and ``COMPLETION_CEILING``, and may add provider-specific request fields through
    ``EXTRA_PAYLOAD``. Instantiating this class directly fails at ``__init__`` rather
    than at the first inference, because an adapter with no destination is a
    programming error and not a runtime condition.
    """

    #: The pinned base URL. Validated for every subclass at import time.
    ENDPOINT: ClassVar[str] = ""
    #: Derived from ``ENDPOINT``; the pinned connection needs it for ``Host`` and for
    #: the TLS handshake name.
    HOSTNAME: ClassVar[str] = ""
    #: How this provider names itself in logs. Never interpolated into a message, only
    #: carried as structured data, so it cannot become a log-injection primitive.
    PROVIDER_LABEL: ClassVar[str] = ""
    #: The largest completion the served model will produce.
    COMPLETION_CEILING: ClassVar[int] = 16_384
    #: Request fields this provider understands and others do not. Sending a field a
    #: provider does not know is not harmless -- some reject the whole request -- so
    #: these stay with the subclass that needs them.
    EXTRA_PAYLOAD: ClassVar[Mapping[str, Any]] = MappingProxyType({})

    def __init_subclass__(cls, **kwargs: Any) -> None:
        """Pin and validate the destination when the subclass module is imported.

        Here rather than in each subclass so that adding a provider cannot skip the
        check by omission: a malformed endpoint fails the process at start-up instead
        of on the first inference, and the hostname is derived from the value that
        was actually validated rather than written out a second time by hand.
        """

        super().__init_subclass__(**kwargs)
        if not cls.__dict__.get("ENDPOINT"):
            return
        validate_fixed_endpoint(cls.ENDPOINT)
        cls.HOSTNAME = urlsplit(cls.ENDPOINT).hostname or ""
    ...
    def __init__(
        ...
        *,
        ...
        if not self.ENDPOINT or not self.HOSTNAME:
            raise ValueError("An adapter subclass must pin an endpoint")
        if not model:
            raise ValueError("A model must be named")
        ...
        self._max_completion_tokens = min(
            self.COMPLETION_CEILING if max_completion_tokens is None else max_completion_tokens,
            self.COMPLETION_CEILING,
        )
    ...
    async def generate(
        ...
        *,
        ...
        payload: dict[str, Any] = {
            ...
            # Whatever this provider understands and the others do not. Applied here
            # rather than merged over the payload afterwards so it can never overwrite
            # the model, the messages or the completion ceiling.
            **self.EXTRA_PAYLOAD,
        }
        if request.tools:
    ...
    async def _post(
        ...
        if response.status_code == 429:
            ...
            logger.warning(
                "The model provider is rate limiting this credential",
                extra={"llm_provider": self.PROVIDER_LABEL, "retry_after": retry_after},
            )
            raise LLMRateLimited()
        if response.status_code == 413:
            # Groq labels this a rate limit, but waiting does not clear it, and the
            # distinction holds for any provider that bills a token budget: the
            ...
            logger.warning(
                "The provider refused the request as too large for this credential",
                extra={"llm_provider": self.PROVIDER_LABEL, "correlation_id": correlation_id},
            )
        ...
        if response.status_code != 200:
            logger.warning(
                "The provider refused the completion",
                extra={
    ...
    def _normalize(
        ...
        return LLMResponse(
            ...
            tool_calls=_tool_calls_of(message, allowed_tool_names, self.PROVIDER_LABEL),
